import { r as registerInstance, h } from './index-dc6f4989.js';
import { o, u, Z, t } from './index-aa88697c.js';
...
const Ku4Mask = class {
  ...
  templateHandler(newTemplate, oldTemplate) {
    ...
    const previousLength = o(previousValue, oldTemplate, char).length;
    const currentLength = o(currentValue, oldTemplate, char).length;
    const nextIndex = previousLength < currentLength ? previousCharIndex + 1 : previousCharIndex;
    this.init().value = u(o(value, oldTemplate, char), newTemplate, char);
    input.setSelectionRange(nextIndex, nextIndex);
  }
  charHandler(newChar, oldChar) {
    const { value, template } = this;
    this.init().value = u(o(value, template, oldChar), template, newChar);
  }
  ...
  handleFocus() {
    ...
    this.value = u(o(this.value, template, char), template, char);
    Z(0).then(() => {
  ...
  handleKeyDown(e) {
    ...
    this.previousValue = hidden ? u(o(e.target.value, template, char), template, char) : e.target.value;
        ...
        navigator.clipboard.readText()
          ...
          if (!banned || !banned.test(data)) {
            ...
            if (selectionStart < selectionEnd) {
              ...
              const unmasked = o(previousValue, template, char);
